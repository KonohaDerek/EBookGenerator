...
namespace EpubBookTests.Services
{
  ...
  public class IThomeServiceTests
  {
    ...
    [TestMethod]
    public async Task GetCreateEbookDtoFromUrlTestAsync()
    {
      Console.WriteLine("GetCreateEbookDtoFromUrlTestAsync");
      // show start time
      var start = DateTime.Now;
      Console.WriteLine($"start time : {start:yyyy-MM-dd HH:mm:ss}");
      var url = "https://ithelp.ithome.com.tw/users/20120491/ironman/2538";
      var ithome = new IThomeService();
      var actual =await ithome.CreateEbookDtoFromUrlAsync(url);
      actual.ToEpub3File();
      ...
      Console.WriteLine($"time difference : {DateTime.Now.Subtract(start).TotalSeconds}");
    }
